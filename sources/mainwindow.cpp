...
void MainWindow::on_pushButton_delete_prev_clicked() {
    ...
    if (last_input_char == " ") {
        ...
        while (reduced_size > 0 &&
               input_label_text[reduced_size - 1] != ' ' && input_label_text[reduced_size - 1] != '(' &&
               input_label_text[reduced_size - 1] != ')' && input_label_text[reduced_size - 1] != '^') {
            reduced_size--;
        }
...
void MainWindow::on_pushButton_op_pow_clicked() {
    ...
    if (is_num_input == true || is_close_bracket_input == true || is_var_input == true || ui->label_input->text() == "0") {
        ui->label_input->setText(ui->label_input->text() + "^");
        is_pow_input = true;
...
//        double initial_step_size = (x_max - x_min) / ui->expression_graph->width();
//        while (variable <= x_max) {
//            int error = convert_infix_to_RPN(str_for_calc, &q_root);
//            if (error == OK) {
//                error = evaluate_expression(q_root, variable, &result);
...
//        ui->expression_graph->graph(0)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, 2));
